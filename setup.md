...
この手順書は **Step 0 〜 Step 15 の全16ステップ**あります。
**1ステップ終わるごとに進捗ファイルへ記録しながら**進めてください。

> **なぜ進捗ファイルを使うのか**
> 全ステップを一息に実行すると、後半へ進むほど前半の文脈が圧縮され、
> 手順の取りこぼしや「やったつもり」が起きます。
> また途中で失敗したとき、どこまで終わっていたか分からなくなり最初からやり直すことになります。
> 進捗ファイルはその両方を防ぎます。

### まず: 進捗ファイルを確認する

作業フォルダに `SETUP_PROGRESS.md` があるか確認してください。

**ある場合（＝2回目以降）**
中身を読み、`[x]` が付いていない**いちばん上のステップから再開**してください。
完了済みのステップは**絶対にやり直さないでください**
（CLAUDE.md の二重追記、hooks の重複登録などの事故になります）。
「Step N から再開します」と1行報告してから進めてください。

**ない場合（＝初回）**
以下の内容で `./SETUP_PROGRESS.md` を作成してから Step 0 に進んでください。

```markdown
# MyContext セットアップ進捗

> 1ステップ終わるごとに `[ ]` → `[x]` に更新する。
> 未解決のまま先に進んだ場合は `[!]` にして、メモに理由を残す。
...
| `[ ]` | Step 0 | 前提チェックとOS確認 |  |  |
| `[ ]` | Step 1 | フォルダ構造の作成 |  |  |
| `[ ]` | Step 2 | .env と .gitignore |  |  |
| `[ ]` | Step 3 | my_profile.md（Q&A対話） |  |  |
| `[ ]` | Step 4 | CLAUDE.md 2枚（グローバル＋プロジェクト） |  |  |
| `[ ]` | Step 5 | hooks スクリプト3本 |  |  |
| `[ ]` | Step 6 | settings.json（hooks有効化） |  |  |
| `[ ]` | Step 7 | auto-save-context.py |  |  |
| `[ ]` | Step 8 | AppLaud（clone / .venv / ffmpeg） |  |  |
| `[ ]` | Step 9 | pipeline.py |  |  |
| `[ ]` | Step 10 | vault-maintenance.py |  |  |
| `[ ]` | Step 11 | MASTER_TASKS.md と memory |  |  |
| `[ ]` | Step 12 | extract_tasks.py の確認 |  |  |
| `[ ]` | Step 13 | Chatwork 朝ブリーフィング |  |  |
| `[ ]` | Step 14 | 公式プラグイン＋ダッシュボード |  |  |
| `[ ]` | Step 15 | 動作確認 |  |  |

## 未解決・要対応

（`[!]` にした項目をここに書き出す。無ければ「なし」）
```

### 進め方のルール

1. ステップは**番号順**に実行する。飛ばさない
2. 1ステップ終わるごとに `SETUP_PROGRESS.md` の該当行を `[x]` にし、完了日時とメモを書く
3. エラーが出たら、その行を `[!]` にして原因をメモに残し、**自力で解決してから**先へ進む
...
> ✅ **Step 1 完了チェック** — `SETUP_PROGRESS.md` の Step 1 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 2 へ進んでください。
...
> ✅ **Step 2 完了チェック** — `SETUP_PROGRESS.md` の Step 2 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 3 へ進んでください。
...
> ✅ **Step 3 完了チェック** — `SETUP_PROGRESS.md` の Step 3 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 4 へ進んでください。
...
> ✅ **Step 4 完了チェック** — `SETUP_PROGRESS.md` の Step 4 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 5 へ進んでください。
...
> ✅ **Step 5 完了チェック** — `SETUP_PROGRESS.md` の Step 5 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 6 へ進んでください。
...
> ✅ **Step 6 完了チェック** — `SETUP_PROGRESS.md` の Step 6 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 7 へ進んでください。
...
> ✅ **Step 7 完了チェック** — `SETUP_PROGRESS.md` の Step 7 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 8 へ進んでください。
...
> ✅ **Step 8 完了チェック** — `SETUP_PROGRESS.md` の Step 8 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 9 へ進んでください。
...
> ✅ **Step 9 完了チェック** — `SETUP_PROGRESS.md` の Step 9 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 10 へ進んでください。
...
> ✅ **Step 10 完了チェック** — `SETUP_PROGRESS.md` の Step 10 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 11 へ進んでください。
...
> ✅ **Step 11 完了チェック** — `SETUP_PROGRESS.md` の Step 11 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 12 へ進んでください。
...
> ✅ **Step 12 完了チェック** — `SETUP_PROGRESS.md` の Step 12 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 13 へ進んでください。
...
> ✅ **Step 13 完了チェック** — `SETUP_PROGRESS.md` の Step 13 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 14 へ進んでください。
...
> ✅ **Step 14 完了チェック** — `SETUP_PROGRESS.md` の Step 14 の行を `[x]` に更新し、
> 完了日時と、気づいた点（スキップした項目・エラー・保留）をメモ欄に書いてから
> Step 15 へ進んでください。
...
```bash
cat ./SETUP_PROGRESS.md       # 全16行が [x] になっているか
...
```powershell
Get-Content .\SETUP_PROGRESS.md  # 全16行が [x] になっているか
...
> ✅ **Step 15 完了チェック** — `SETUP_PROGRESS.md` の Step 15 を `[x]` にし、
> **全16行が `[x]` になっているか**を確認してください。
> `[!]` が残っている場合は「セットアップ完了」と報告せず、
> 未解決の項目を一覧にして報告してください。
